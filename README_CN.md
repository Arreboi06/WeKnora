...
题目四围绕可审计的 **Wiki 来源证据画像** 实现：每个生成的知识视图都必须能够追溯到产生它的完整来源全集。设计把关键失效场景明确固化为不变量：来源变化会使完整的依赖投影失效，并拒绝不完整回填；并发解析的陈旧结果不能发布；迁移失败会在应用对外提供服务前阻止启动；多 scope 操作遵循确定性的锁顺序；ACL 状态由权威 marker 同步并带有失效栅栏；SQLite 与 PostgreSQL 的索引语义通过一致性检查对齐。

实现与验证入口集中在 [`docs/rhino-2026-t4-design.md`](./docs/rhino-2026-t4-design.md)。验证覆盖真实 PostgreSQL 并发/API、重启与隔离备份恢复、真实 App + DB + browser 链路、Go/vet 与前端检查以及秘密扫描。提交实现快照固定在 [`rhino-2026-final-T4`](https://github.com/Arreboi06/WeKnora/tree/rhino-2026-final-T4)，代码提交为 `0bbb4853cb4bab38e1e947a1ec6b9d0178b40794`。

后续计划保持可度量：取得两份独立真人标注并完成裁决，以闭合有效性评估；在具备所需外部模块 payload 的环境中重新执行容器 CGO=0 路径。后续工作不改变已提交行为和上述来源真相不变量。
